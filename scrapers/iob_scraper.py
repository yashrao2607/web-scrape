...
class IOBScraper(BaseScraper):
    """
    Scraper implementation for Indian Overseas Bank (IOB).
    """
    async def scrape(self, page: Any) -> Dict[str, Any]:
        self.logger.info("starting_iob_scrape")
        ...
        # Post-process to add the 0.50% senior citizen premium since IOB footnotes specify
        # that senior citizens receive an additional 0.50% p.a. over public card rates.
        for item in rates:
            try:
                gen_rate_str = item.get("general_raw", "").replace("%", "").strip()
                gen_rate_val = float(gen_rate_str)
                # Apply 0.50% premium
                item["senior_raw"] = f"{gen_rate_val + 0.50:.2f}%"
            except Exception:
                pass
