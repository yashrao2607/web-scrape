...
export class BankOfIndiaScraper extends BaseScraper {
  ...
  async parseRatesFromText(page) {
    ...
    for (const line of lines) {
      ...
      if (/rupee term deposit|domestic.*nro.*rate|maturity.*less than.*3.*cr/i.test(lc)) {
        inFDSection = true;
      }
      if (/non-callable|nre|fcnr|premature|green deposit|recurring/i.test(lc)) {
        inFDSection = false;
      }
    ...
    for (const line of fdSection) {
      ...
      parsed.push({
        ...
        section_name: "Rupee Term Deposits (<3 Cr)",
        table_name: "",
