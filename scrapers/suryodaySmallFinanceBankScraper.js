...
export class SuryodaySmallFinanceBankScraper extends BaseScraper {
  async scrape(page) {
    this.logger.info("starting_suryoday_small_finance_bank_scrape");
    let rates = [];
    ...
    // Also drop the NRE/NRI table so retail resident rates aren't overridden.
    tables = tables.filter(t => !/highlight|credit rating|\bnre\b|\bnri\b/i.test(t.section_name || ""));
